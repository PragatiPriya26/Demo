...
const borderColors = ["border-l-sage", "border-l-blush", "border-l-lavender", "border-l-gold", "border-l-sage-dark"];
...
const AffirmationsSection = () => (
  <section className="px-6 md:px-12 py-16 md:py-20 bg-lavender-light">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div>
        <h3 className="font-display text-lg mb-4 text-ink">Words of Affirmation</h3>
      ...
      <motion.div
        ...
        viewport={{ once: true }}
      >
        <p className="font-display text-[22px] text-ink mb-1.5">📖 Your Journal</p>
        <p className="text-xs text-light italic mb-4">Friday, March 27 · Private & Secure</p>
        <p className="text-[13px] text-light mb-4">Today's prompt</p>
        ...
        <div className="flex flex-col gap-7 mb-5">
          <div className="h-px bg-sage/40 relative">
            <span className="absolute -top-2.5 left-0 text-light/60 font-handwriting text-base">Start writing…</span>
          </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {["💭 What am I grateful for?", "🌱 What challenged me?", "✨ What was joyful?", "🌙 Tonight I let go of…"].map((p) => (
            <span key={p} className="px-3.5 py-1.5 rounded-full bg-warm text-xs text-mid cursor-pointer border border-transparent hover:border-sage hover:text-sage-dark transition-all">
              {p}
            </span>
          ))}
        </div>
